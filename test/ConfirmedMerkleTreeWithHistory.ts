...
import { ConfirmedMerkleTreeWithHistory } from "../typechain-types/ConfirmedMerkleTreeWithHistory";
import { ethers } from "hardhat";

describe("ConfirmedMerkleTreeWithHistory", () => {
  let confirmedMerkleTreeUtils: CMTUtils;
  let contract: ConfirmedMerkleTreeWithHistory;
  ...
  const deployNewMerkleRootContract = async () => {
    const ConfirmedMerkleTree = await ethers.getContractFactory("ConfirmedMerkleTreeWithHistory", {
      libraries: {
  ...
  it("should return merkle root for 5 data", async () => {
    await contract.append(ethers.utils.hexZeroPad("0x1", 32));
    await contract.append(ethers.utils.hexZeroPad("0x2", 32));
    await contract.append(ethers.utils.hexZeroPad("0x3", 32));
    ...
    expect(await contract.merkleRoot()).to.equal(calculateMerkleRoot("0x01", "0x02", "0x03", "0x04", "0x05"));
  });

  it("should return merkle root for 6 data", async () => {
    await contract.append(ethers.utils.hexZeroPad("0x1", 32));
    await contract.append(ethers.utils.hexZeroPad("0x2", 32));
    await contract.append(ethers.utils.hexZeroPad("0x3", 32));
    await contract.append(ethers.utils.hexZeroPad("0x4", 32));
    ...
    expect(await contract.merkleRoot()).to.equal(calculateMerkleRoot("0x01", "0x02", "0x03", "0x04", "0x05", "0x06"));
  });

  it("should return merkle root for 7 data", async () => {
    await contract.append(ethers.utils.hexZeroPad("0x1", 32));
    await contract.append(ethers.utils.hexZeroPad("0x2", 32));
    await contract.append(ethers.utils.hexZeroPad("0x3", 32));
    await contract.append(ethers.utils.hexZeroPad("0x4", 32));
    ...
    expect(await contract.merkleRoot()).to.equal(
      calculateMerkleRoot("0x01", "0x02", "0x03", "0x04", "0x05", "0x06", "0x07")
    );
  });

  it("should return merkle root for 9 data", async () => {
    await contract.append(ethers.utils.hexZeroPad("0x1", 32));
    await contract.append(ethers.utils.hexZeroPad("0x2", 32));
    await contract.append(ethers.utils.hexZeroPad("0x3", 32));
    await contract.append(ethers.utils.hexZeroPad("0x4", 32));
    ...
    expect(await contract.merkleRoot()).to.equal(
      calculateMerkleRoot("0x01", "0x02", "0x03", "0x04", "0x05", "0x06", "0x07", "0x08", "0x09")
    );
  });

  it("should success for 128-256 data", async () => {
    const dataSize = choiceRandom(128, 256);
